...
export default function App() {
  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        ...
        // 如果連快取都沒有，才顯示錯誤提示
        if (!localStorage.getItem('gasDataCache')) {
  ...
  const handleRegister = (form) => {
    ...
    if (users.some(u => u.phoneLast5 === form.phoneLast5)) { 
      setAuthError('此手機末五碼已被註冊'); 
      return false; // 回傳 false，讓 LoginScreen 組件不執行跳轉
    }

    // 2. 成功路徑 (跳轉回登入頁面並顯示成功訊息)
    const newUser = { 
      ...form, 
      id: 'U_' + form.phoneLast5 + '_' + Date.now(), 
      role: 'user', 
      status: 'active' 
    };
    ...
    // 【背景處理】
     api.addUser(newUser).catch(e => console.error('註冊失敗:', e));
    
    return true; // 回傳 true，讓 LoginScreen 自動切換回登入模式
  };
  ...
  const handleAddUser = (newUserForm) => {
    ...
    // 【背景處理】
    api.addUser(newUser).catch(e => console.error('新增會員失敗:', e));
  };
  ...
  const handleUpdateResStatus = (resId, status) => {
    ...
    // 【背景處理】
    api.updateStatus(resId, status).catch(e => console.error('更新狀態失敗:', e));
    return true;
  };
  ...
  const addToCart = (item) => {
    ...
    setCart(prev => [...prev, { ...item, borrowQty: 1, startDate: '', endDate: '', startTime: '', endTime: '',
      maxQty: currentAvailableQty > 0 ? currentAvailableQty : (item.qty || 1)
    }]);
  ...
  const submitReservation = () => { 
    ...
    // 【背景處理】
    api.addReservation(newReservation).catch(e => console.error('送出申請失敗:', e));
  };
  ...
  const addItem = (d) => {
    ...
    // 【背景處理】
    api.addItem(newItem).catch(e => console.error('上架失敗:', e));
  };

  // 1. 新增貼文
  const addManualNews = (newsForm) => {
    const newObj = {
      ...newsForm,
      id: `NW_${Date.now()}`,
      date: new Date().toLocaleString('zh-TW', {hour12: false})
    ...
    // 【樂觀更新】不等待 API，瞬間更新畫面與顯示成功提示！
    setNews(prev => [...prev, newObj]);
    showAlert('最新消息發佈成功！');

    // 【背景處理】把資料丟給 GAS 慢慢處理，不卡住畫面
    api.addNews(newObj).catch(e => {
      console.error('同步至資料庫失敗:', e);
      // 如果極低機率失敗了，這裡可以選擇跳出錯誤通知
    });
  };

  // 2. 編輯貼文
  const handleUpdateNews = (newsForm) => {
    // 【樂觀更新】瞬間替換掉畫面上的舊資料
    setNews(prev => prev.map(n => n.id === newsForm.id ? newsForm : n));
    showAlert('消息更新成功！');

    // 【背景處理】
    api.updateNews(newsForm).catch(e => console.error('同步至資料庫失敗:', e));
  };
  ...
  const handleDeleteNews = (newsId) => {
    ...
    // 【背景處理】
    api.deleteNews(newsId).catch(e => console.error('同步至資料庫失敗:', e));
  };
  ...
  // 1. 定義統一的「任務項目」過濾邏輯：包含「審核中」、「已退回」或「實質已逾期」
  const isTaskItem = useCallback((res) => {
    if (!res) return false;
    // 判斷標準：狀態為審核中、資料庫標記為已逾期，或是時間判定已逾期
    return res.status === '審核中' || res.status === '已退回' || res.status === '已逾期' || checkIsOverdue(res);
  }, []);
        ...
        />
          ...
          <main className={`flex-1 ${isSimulatingUser ? 'pt-[168px]' : 'pt-24'} pb-12 px-4 md:px-8 max-w-7xl mx-auto w-full z-0 relative`}>
            <div key={activeTab} className="page-reveal">
              {activeTab === 'dashboard' && <UserDashboard items={items} itemTypes={itemTypes} reservations={reservations} onAddToCart={addToCart} isSimulatingUser={isSimulatingUser} currentUser={currentUser} userHasOverdue={userHasOverdue} isMobileDevice={isMobileDevice} cartAnimObj={cartAnimObj} />}
              {activeTab === 'items' && <UserDashboard items={items} itemTypes={itemTypes} reservations={reservations} onAddToCart={addToCart} isSimulatingUser={isSimulatingUser} currentUser={currentUser} userHasOverdue={userHasOverdue} isMobileDevice={isMobileDevice} cartAnimObj={cartAnimObj} />}
